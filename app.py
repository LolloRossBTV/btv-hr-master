...
if not st.session_state.autenticato:
    ...
    if scelta == "📊 Dashboard":
        ...
        if not mie_richieste.empty:
            ...
            else: st.info("Non hai prenotazioni dalla prossima settimana.")
        else: st.info("Nessuna prenotazione trovata.")
                ...
                if len(p) == 2:
                    ...
                    for g in giorni_r:
                        if str(g.date()) in df_blocchi['Data'].astype(str).values:
                            st.error(f"❌ Il {g.date()} è bloccato."); possibile = False; break
    ...
    # --- ADMIN ---
        ...
        with t3: # Utenti & RESET & ELIMINA
            col_left, col_right = st.columns(2)
            
            with col_left:
                st.subheader("Reset Password")
                u_reset = st.selectbox("Seleziona utente per RESET PW", ["---"] + df_dip['Nome'].tolist(), key="reset_sel")
                if st.button("RESETTA A 12345"):
                    if u_reset != "---":
                        ...
                        conn.update(worksheet="Dipendenti", data=df_dip.drop(columns=['Nome_Display']))
                        st.success(f"PW Reset per {u_reset}"); st.rerun()
            
            with col_right:
                st.subheader("Elimina Utente")
                u_del = st.selectbox("Seleziona utente da ELIMINARE", ["---"] + df_dip['Nome'].tolist(), key="del_sel")
                if st.button("ELIMINA DEFINITIVAMENTE", type="primary"):
                    if u_del != "---":
                        df_final = df_dip[df_dip['Nome'] != u_del].drop(columns=['Nome_Display'])
                        conn.update(worksheet="Dipendenti", data=df_final)
                        st.success(f"{u_del} rimosso correttamente!"); st.rerun()
        ...
        with t4: # Blocchi
            st.write("**Blocca date specifiche**")
            with st.form("block"):
                ...
                if st.form_submit_button("BLOCCA"):
                    ...
                    conn.update(worksheet="Blocchi", data=pd.concat([df_blocchi, nuovo_b], ignore_index=True)); st.rerun()
            st.write("**Date bloccate:**")
            st.dataframe(df_blocchi)
            if st.button("Svuota Blocchi"):
